...
def export_results(
    ...
    for batch_idx in range(input_data.image.size(0)):
        ...
        # Save images
        _save_images(result, batch_idx, sample_dir, incremental=incremental, save_separate_images=save_separate_images)
        
        # _save_test_data_video(result, batch_idx, sample_dir, incremental=incremental)
...
def create_video_from_image_sequence(image_folder_path, output_video_path, framerate=30, image_pattern="*.png"):
    """
    Create video from a sequence of images in a folder.
    
    Args:
        image_folder_path (str): Path to folder containing image sequence
        output_video_path (str): Path to save the output video
        framerate (int): Frames per second for the video
        image_pattern (str): Pattern to match image files (e.g., "*.png", "*.jpg")
    """
    import glob
    from PIL import Image
    
    # Get all image files matching the pattern
    image_files = glob.glob(os.path.join(image_folder_path, image_pattern))
    
    if not image_files:
        print(f"Warning: No images found in {image_folder_path} with pattern {image_pattern}")
        return
    
    # Sort files naturally (handles 001.png, 002.png, etc.)
    ...
    if not frames:
        print(f"Error: No valid images could be loaded from {image_folder_path}")
        return
    # Create video from frames
    data_utils.create_video_from_frames(frames, output_video_path, framerate=framerate)
    print(f"Video saved to: {output_video_path} ({len(frames)} frames)")
...
def _save_images(result, batch_idx, out_dir, incremental=-1, save_separate_images=False):
    ...
    input_img = result.target.image[batch_idx]  # [v, c, h, w]
    num_gt_views = input_img.size(0)
    for v in range(num_gt_views):
        single_gt = input_img[v]  # [c, h, w]
        single_gt = (single_gt.cpu().numpy() * 255.0).clip(0.0, 255.0).astype(np.uint8)
        single_gt = np.transpose(single_gt, (1, 2, 0))  # [h, w, c]
        if incremental == -1:
            Image.fromarray(single_gt).save(os.path.join(out_dir, f"input_{v}.png"))
        else:
            Image.fromarray(single_gt).save(os.path.join(out_dir, f"input_{v}_{incremental}.png"))

    if save_separate_images:
        # Save GT images - each view separately
        gt_img = result.target.image[batch_idx]  # [v, c, h, w]
        num_gt_views = gt_img.size(0)
        for v in range(num_gt_views):
            single_gt = gt_img[v]  # [c, h, w]
            single_gt = (single_gt.cpu().numpy() * 255.0).clip(0.0, 255.0).astype(np.uint8)
            single_gt = np.transpose(single_gt, (1, 2, 0))  # [h, w, c]
            if incremental == -1:
                Image.fromarray(single_gt).save(os.path.join(out_dir, f"gt_view_{v}.png"))
            else:
                Image.fromarray(single_gt).save(os.path.join(out_dir, f"gt_view_{v}_{incremental}.png"))
        
        # Save prediction images - each view separately
        pred_img = result.render[batch_idx]  # [v, c, h, w]
        num_pred_views = pred_img.size(0)
        for v in range(num_pred_views):
            single_pred = pred_img[v]  # [c, h, w]
            single_pred = (single_pred.to(torch.float32).cpu().numpy() * 255.0).clip(0.0, 255.0).astype(np.uint8)
            single_pred = np.transpose(single_pred, (1, 2, 0))  # [h, w, c]
            if hasattr(result.target, "name"):
                name = f'apred_{result.target.name[v]}.png'
            else:
                name = f"apred_{v}.png"
            if incremental != -1:
                name = f"{name}_{incremental}.png"
            Image.fromarray(single_pred).save(os.path.join(out_dir, name))
    else:
        # Save GT vs prediction side-by-side (original behavior)
