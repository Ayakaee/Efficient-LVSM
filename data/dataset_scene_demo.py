...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.inference and scene_name in self.view_idx_list:
            ...
            vt = self.config.training.num_target_views
            # target = list(range(0, vt))
            # target = np.linspace(0, len(frames)-1, vt, dtype=int).tolist()
            target = current_view_idx["target"]
            image_indices= current_view_idx["context"] + target
        else:
