...
class Dataset(Dataset):
    def __init__(self, config):
        ...
        if self.inference:
            ...
            if self.config.inference.get("view_idx_file_path", None) is not None \
                and os.path.exists(self.config.inference.view_idx_file_path):
                with open(self.config.inference.view_idx_file_path, 'r', encoding='utf-8') as f:
    ...
    def __getitem__(self, idx):
        ...
        if self.inference and scene_name in self.view_idx_list:
            ...
            v_input = self.config.training.num_input_views
            v_target = self.config.training.num_target_views
            
            if v_input > len(current_view_idx["context"]):
                v_st, v_end = current_view_idx["context"]
                views = np.rint(np.linspace(v_st, v_end, v_input)).astype(int).tolist()
                views[0] = v_st
                views[-1] = v_end
                current_view_idx["context"] = views
            
            if v_input == 1:
                image_indices= current_view_idx["context"][:1] + current_view_idx["target"][:v_target]
            else:
                image_indices = current_view_idx["context"][:v_input - 1] + current_view_idx["context"][-1:] + current_view_idx["target"][:v_target]
            
            
            # print('origin', image_indices)
            # image_indices[0] = min(image_indices[0], image_indices[2])
            # image_indices[1] = max(image_indices[1], image_indices[-1])
            # print('after', image_indices)
        else:
